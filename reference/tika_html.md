...
If `output_dir` is specified, files will have the `.html` file
extension.
...
`input`, of unparsed `XHTML`. Unprocessed files are `as.character(NA)`.
...
#> Error in tika(input = input, output = "html", ...): !any(is.na(jar)) is not TRUE
# }
```
