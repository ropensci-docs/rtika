...
To start, you need R and `Java 11` (or newer, e.g. OpenJDK 17+). Higher
versions work. To check your version, run the command `java -version`
from a terminal. Get Java installation tips at
...
- [`tika_xml()`](https://docs.ropensci.org/rtika/reference/tika_xml.md)
  ...
  to get a structured XHMTL rendition.
- [`tika_json()`](https://docs.ropensci.org/rtika/reference/tika_json.md)
  to get metadata as `.json`, with XHMTL content.
- [`tika_json_text()`](https://docs.ropensci.org/rtika/reference/tika_json_text.md)
  to get metadata as `.json`, with plain text content.
- [`tika()`](https://docs.ropensci.org/rtika/reference/tika.md) is the
  main function the others above inherit from.
...
recommend batches. Currently, the
[`tika()`](https://docs.ropensci.org/rtika/reference/tika.md) parsers
take a tiny bit of time to spin up, and that will get annoying with
hundreds of separate calls to the functions.
...
include `rTika` as a dependency and call the
[`rtika::tika_jar()`](https://docs.ropensci.org/rtika/reference/tika_jar.md)
...
the [`tabulapdf`](https://github.com/ropensci/tabulapdf) package by
